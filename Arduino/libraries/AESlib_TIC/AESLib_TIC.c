...
// encrypt multiple blocks of 128bit data, data_len but be mod 16. If data_len isn't mod 16, it will auto plus char
// key is assumed to be 128bit thus 16 uint8_t's

void aes128_enc_multiple_plus(const uint8_t* key, void* data ,const uint16_t data_len, char* c_str){
	// uint16_t data_len = strlen(data);
	uint16_t data_len_buf = data_len;
	if (data_len_buf % 16 != 0) {
		data_len_buf = 16 - (data_len_buf % 16);
		for(int i=0; i < data_len_buf ; i++){
			*strcat(data,c_str);
		  // *strcat(data,c_str); 
		}
		...
		// *data=data;
		// strcat(data,c_str);
		// data_len_buf = sizeof(data)-1;
		// data_len_buf = ((data_len/16) + 1)*16;
		data_len_buf=96;
		aes128_enc_multiple(key,data,data_len_buf);
	}
	else{
		aes128_enc_multiple(key,data,data_len);
	}
	// aes128_ctx_t ctx;
	// aes128_init(key, &ctx);
...
// key is assumed to be 128bit thus 16 uint8_t's
void aes128_dec_multiple(const uint8_t* key, void* data,uint16_t data_len){
	// uint16_t data_len_buf = data_len;
	if (data_len % 16 != 0) {
...
// decrypt multiple blocks of 128bit data, data_len must be mod 16.
// key is assumed to be 128bit thus 16 uint8_t's
// void aes128_dec_multiple_plus(const uint8_t* key, void* data, const uint16_t data_len){
// 	if (data_len % 16 != 0) {
// 		return;
// 	}
// 	aes128_ctx_t ctx;
